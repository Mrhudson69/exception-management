...
/** Renders the custom logo if set, otherwise the default hub glyph. */
export function BrandLogo({ logo, size = 32 }: { logo: string | null; size?: number }) {
  if (logo) {
    return (
      <img
        src={logo}
  ...
  return (
    <div className="rounded-full bg-primary-container flex items-center justify-center shrink-0" style={{ width: size, height: size }}>
      <Icon name="hub" size={Math.round(size * 0.56)} className="text-on-primary-container" />
    </div>
  );
}
    ...
    >
      <div className="px-container-padding pb-6 mb-4 border-b border-outline-variant">
        <div className="flex items-center gap-3">
          <BrandLogo logo={branding.logo} size={32} />
          <div className="flex-1 min-w-0">
            <h1 className="text-headline-md font-bold text-primary leading-tight truncate">{branding.appName}</h1>
  ...
  return (
    <header className="bg-surface h-14 w-full sticky top-0 z-30 border-b border-outline-variant flex justify-between items-center px-4 sm:px-container-padding shrink-0 gap-2">
      <div className="flex items-center gap-2 sm:gap-6 h-full min-w-0">
        ...
        <span className="text-title-sm sm:text-headline-md font-bold text-primary tracking-tight truncate">{branding.productName}</span>
